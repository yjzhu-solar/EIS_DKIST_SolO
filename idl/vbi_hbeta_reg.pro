pro vbi_hbeta_reg, kernel_size = kernel_size

vbi_hbeta_dir = '../src/DKIST/vbi_1024/BJOLO/'
vbi_files = file_search(vbi_hbeta_dir + '*.fits')
n_files = n_elements(vbi_files)
vbi_hbeta_sav_dir = '../sav/DKIST_reg/BJOLO/'
...
for ii = 0, n_files - 2 do begin
    ...
    save, filename=vbi_hbeta_sav_dir + file_basename(vbi_files[ii], '.fits') + '.sav', im_ref

    if ii eq n_files - 2 then begin
        im_ref = im_reg
        save, filename=vbi_hbeta_sav_dir + file_basename(vbi_files[ii+1], '.fits') + '.sav', im_ref
    endif
