...
if os.path.exists ("../../src/EUI/HRI/euv174/20221024/coalign_shifts_step_all.h5"):
    with h5py.File("../../src/EUI/HRI/euv174/20221024/coalign_shifts_step_all.h5","r") as f:
    ...
    for ii in range(180,-1,-1):
        eui_map_seq_segment_ = eui_map_seq[ii:ii+2]
        eui_map_seq_shift_ = coalignment.calculate_match_template_shift(eui_map_seq_segment_,layer_index=-1)
        eui_map_seq_coalign_shifts_x[ii] = eui_map_seq_shift_["x"].value[0] + eui_map_seq_coalign_shifts_x[ii+1]
        eui_map_seq_coalign_shifts_y[ii] = eui_map_seq_shift_["y"].value[0] + eui_map_seq_coalign_shifts_y[ii+1]

    for ii in range(182,len(eui_files)):
        eui_map_seq_segment_ = eui_map_seq[ii-1:ii+1]
        eui_map_seq_shift_ = coalignment.calculate_match_template_shift(eui_map_seq_segment_,layer_index=0)
        eui_map_seq_coalign_shifts_x[ii] = eui_map_seq_shift_["x"].value[-1] + eui_map_seq_coalign_shifts_x[ii-1]
        eui_map_seq_coalign_shifts_y[ii] = eui_map_seq_shift_["y"].value[-1] + eui_map_seq_coalign_shifts_y[ii-1]

    with h5py.File("../../src/EUI/HRI/euv174/20221024/coalign_shifts_step_all.h5","w") as f:
...
for ii, map in enumerate(eui_map_seq_coalign[:]):
    ...
    plt.savefig(fname=os.path.join("../../figs/EUI/20221024/upflow_video_step_all/",f"eui_hri_20221024_{ii:03d}.png"),
                dpi=300)
    fig.clf()
    plt.close(fig)
