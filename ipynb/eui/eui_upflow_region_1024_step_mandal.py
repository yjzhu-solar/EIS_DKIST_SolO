...
if os.path.exists ("../../src/EUI/HRI/euv174/20221024/coalign_shifts_step_mandal.h5") and not overwrite_coalign:
    ...
    n_segment = 15
    len_segment = len(eui_files)//n_segment

    eui_map_seq_coalign_shifts_x = np.zeros(len(eui_files))
    eui_map_seq_coalign_shifts_y = np.zeros(len(eui_files))

    for ii_seg in tqdm(range(n_segment)):
        start_index_ = ii_seg*len_segment
        if ii_seg == n_segment-1:
            end_index_ = (ii_seg+1)*len_segment
        else:
            end_index_ = (ii_seg+1)*len_segment + 1

        eui_map_seq_seg_ = eui_map_seq[start_index_:end_index_]
        eui_map_seg_template_ = eui_map_seq_seg_[0].submap([650,100]*u.pix,
                                        top_right=[1300,750]*u.pix)
        eui_map_seq_coalign_shifts_ = coalignment.calculate_match_template_shift(eui_map_seq_seg_,
                                                                                 template=eui_map_seg_template_,
                                                                                 layer_index=0,
                                                                                 func=np.log10)
        

        eui_map_seq_coalign_shifts_x[start_index_:end_index_] = eui_map_seq_coalign_shifts_["x"].value + eui_map_seq_coalign_shifts_x[start_index_]
        eui_map_seq_coalign_shifts_y[start_index_:end_index_] = eui_map_seq_coalign_shifts_["y"].value + eui_map_seq_coalign_shifts_y[start_index_]
    
    eui_map_seq_coalign_shifts_x = eui_map_seq_coalign_shifts_x - eui_map_seq_coalign_shifts_x[181]
    eui_map_seq_coalign_shifts_y = eui_map_seq_coalign_shifts_y - eui_map_seq_coalign_shifts_y[181]

    with h5py.File("../../src/EUI/HRI/euv174/20221024/coalign_shifts_step_mandal.h5","w") as f:
...
if not os.path.exists("../../figs/EUI/20221024/upflow_video_step_mandal/"):
    os.makedirs("../../figs/EUI/20221024/upflow_video_step_mandal/")

for ii, map in enumerate(tqdm(eui_map_seq_coalign[:])):
    ...
    plt.savefig(fname=os.path.join("../../figs/EUI/20221024/upflow_video_step_mandal/",f"eui_hri_20221024_{ii:03d}.png"),
