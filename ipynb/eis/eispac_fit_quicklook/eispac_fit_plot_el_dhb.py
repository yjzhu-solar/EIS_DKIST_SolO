...
def eispac_fit_plot(fitres_path, index, figsize=(12,4),save_dir=None,format="png",dpi=300, inst_width="eis_software",
    ...
    clb1, clb_ax1 = plot_colorbar(im=ax1.images[0],ax=ax1,width="10%",bbox_to_anchor=(1.05, 0., 1, 1),fontsize=10)
    
    ax1.set_title("Fe XII 19.51 nm Intensity",pad=20)
    ...
    clb2, clb_ax2 = plot_colorbar(im=ax2.images[0],ax=ax2,width="10%",bbox_to_anchor=(1.05, 0., 1, 1),fontsize=10)

    ax2.set_title("Doppler Shift [km/s]",pad=20)
    ...
    clb3, clb_ax3 = plot_colorbar(im=ax3.images[0],ax=ax3,width="10%",bbox_to_anchor=(1.05, 0., 1, 1),fontsize=10)

    ax3.set_title("Nonthermal Velocity [km/s]",pad=20)

    for clb_ax_ in (clb_ax1,clb_ax2,clb_ax3):
        clb_ax_.tick_params(axis="y",labelbottom=False,labeltop=True,top=True,bottom=False)
        clb_ax_.ticklabel_format(axis="y",style="sci",scilimits=(0,2),useMathText=True)
